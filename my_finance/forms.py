...
from .models import PropertyRentalInfo   
class PropertyRentalInfoForm(forms.ModelForm):
    class Meta:
...
class ExpenseForm(forms.ModelForm):
    ...
    expense_date = forms.CharField(
        widget=forms.TextInput(
            attrs={"class": "form-control flatpickr-basic", "placeholder": "YYYY-MM-DD"}
        ),
        required=True,
    )
...
class BudgetForm(forms.ModelForm):
    currency = forms.CharField(
        widget=forms.Select(choices=CURRENCIES, attrs={"class": "form-control"})
    )
    auto_budget = forms.CharField(widget=forms.CheckboxInput(attrs={"class": "info"}))
    budget_period = forms.CharField(
        initial="Monthly",
        widget=forms.Select(
    ...
    def __init__(self, *args, **kwargs):
        try:
            ...
            self.fields["categories"] = forms.ModelChoiceField(
                queryset=Category.objects.filter(user=user_name).exclude(
                    name__in=["Goals", "Funds"]
                ),
                empty_label="Select Category Group",
                widget=forms.Select(
                    attrs={
                        "class": "form-control pick_category",
            ...
            self.fields["user_budget"] = forms.ModelChoiceField(
                ...
                widget=forms.Select(
                    attrs={
                        "class": "form-control user_budget",
                        "method_name": "add_budget",
                    }
                ),
            )
...
class TemplateBudgetForm(forms.ModelForm):
    ...
    auto_budget = forms.CharField(widget=forms.CheckboxInput(attrs={"class": "info"}))
    budget_period = forms.CharField(
        widget=forms.Select(choices=BUDGET_PERIODS, attrs={"class": "form-control"})
    )
...
class BillForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["user_budget"] = forms.ModelChoiceField(
            queryset=UserBudgets.objects.filter(user=user_name),
            empty_label="Select User Budget",
            widget=forms.Select(attrs={"class": "form-control user_budget"}),
        )
...
class TransactionForm(forms.ModelForm):
    ...
    cleared = forms.CharField(
        widget=forms.CheckboxInput(attrs={"class": "info"}), required=False
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["category"] = forms.ModelChoiceField(
            queryset=Category.objects.filter(
                user=user_name
            ),  # .exclude(name__in=["Goals"]),
            empty_label="Select Category Group",
            widget=forms.Select(attrs={"class": "form-control pick_category"}),
        )
        self.fields["user_budget"] = forms.ModelChoiceField(
            queryset=UserBudgets.objects.filter(user=user_name),
            empty_label="Select User Budget",
        ...
        self.fields["bill"] = forms.ModelChoiceField(
            queryset=Bill.objects.filter(user=user_name),
...
class AccountForm(forms.ModelForm):
    ...
    include_net_worth = forms.CharField(
        widget=forms.CheckboxInput(attrs={"class": "info"})
    )
...
class LiabilityForm(forms.ModelForm):
    ...
    liability_type = forms.CharField(
        widget=forms.Select(choices=MORTGAGE_TYPES, attrs={"class": "form-control"})
    )
    interest_period = forms.CharField(
        widget=forms.Select(choices=INTEREST_PERIODS, attrs={"class": "form-control"})
    )
