...
from .utils import check_subscription_status, process_ai_feature
...
def ai_user_page(request):
    ...
    try:
        ...
        usage_records = AIUserFeatureUsage.objects.filter(
        ...
        available_plans = (
            AISubscriptionPlan.objects.exclude(id=subscription.plan.id)
            if subscription.plan
            else AISubscriptionPlan.objects.all()
        )
...
def ai_admin_page(request):
    ...
    for plan in plans:
        ...
        active_count = AIUserSubscription.objects.filter(
