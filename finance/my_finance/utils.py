...
from .models import UserFeatureUsage, UserSubscription
...
def get_or_create_usage_record(user, feature_name):
    try:
        ...
        period_start, period_end = compute_current_period(
            user_subscription.registration_date, 
            timezone.now()
        )
...
def process_ai_feature(user, feature_name):
    ...
    allowed_limit = settings.SUBSCRIPTION_PLANS.get(plan_name, {}).get(feature_name, 0)

    if usage.usage_count >= allowed_limit:
        return {"error": "Monthly usage limit reached for this feature."}

    with transaction.atomic():
        updated = UserFeatureUsage.objects.filter(
            id=usage.id,
            usage_count__lt=allowed_limit
        ).update(usage_count=F('usage_count') + 1)
        
        if not updated:
            return {"error": "Could not update usage count. Please try again."}

    return {"message": "AI feature executed successfully.", "remaining": allowed_limit - (usage.usage_count + 1)}
