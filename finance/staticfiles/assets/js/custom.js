$(document).ready(function () {
    ...
    $(".download_rental_pdf").on("click", function (e) {
        ...
        var total_return_investor_data = $(this).attr(
            "total_return_investor_data"
        );
        var net_operating_income_data = $(this).attr(
        ...
        var return_on_investment_data = $(this).attr(
    ...
    $("body").delegate(".download_csv", "click", function (event) {
        ...
        if (pdf_title) {
            formHtml +=
                "<input type='hidden' name='pdf_title' value='" +
                pdf_title +
                "' >";
        }
    ...
    $("body").delegate(".delete_button", "click", function (event) {
        ...
        if (del_method == "account_delete") {
            text_msg +=
                "All the transactions related to this account also deleted.";
        }
        ...
        if (del_method == "bill_delete") {
            text_msg +=
                "All the transactions, paid or unpaid bills also deleted.";
        }
        if (del_method == "selected_bill_delete") {
            text_msg +=
                "All the transactions related to this bills also deleted.";
        }
    ...
    function make_date_str(date_value) {
        var month_date = date_value.toLocaleString("default", {
            month: "long",
        });
        var result_date =
            month_date +
    ...
    function duration_calculate() {
        ...
        if (new Date(lease_start_date) >= new Date(lease_end_date)) {
            ...
            $(".rent_end_date").text(
                "*Please choose greater date from start date"
            );
            ...
            if ($(".rent_end_date").text() != "Month-to-Month") {
                ...
                rent_duration = make_rent_duration(
                    lease_start_date,
                    lease_end_date
                );
    ...
    $("body").delegate(".change_rent_amount", "change", function (event) {
        ...
        currency_symbol = $("#currency_name").val();
        $(".rent_amount_month").text(
            currency_symbol + rental_amount + "/month"
        );
        $("#rental_invoice").empty();
        $(".change_every_month_date").val("Select");
        $(".due_date_rental").text("N/A");
    ...
    $("body").delegate("#rental_invoice", "change", function (event) {
        ...
        if (term_name == "Fixed Term") {
            ...
            var every_month_value = parseInt(
                $(".change_every_month_date").val()
            );
    ...
    $("body").delegate(".edit_schedule", "click", function (event) {
        ...
        if (
            ...
            var invoice_amount_list = JSON.parse(
                $("#invoice_amount_list").val()
            );
            ...
            if (title_name == "Save") {
                total_rental_amount =
                    parseFloat(total_rental_amount) -
                    old_amount_value +
                    new_amount_value;
            ...
            success: function (data) {
                ...
                for (let i = 0; i < unit_list.length; i++) {
                    if (i == 0) {
                        ...
                        $("#tenant_n").attr(
                            "tenant_dict",
                            JSON.stringify(tenant_dict)
                        );
                    }
                    optionHtml +=
                        "<option>" + unit_list[i]["name"] + "</option>";
                }
                console.log();
                $("#unit_options").append(optionHtml);
    ...
    $("body").delegate(".show_due_bills", "change", function (event) {
        ...
        //    }
        if (
            category_name == "Bills & Subscriptions" ||
            category_name == "Bills"
        ) {
            $.ajax({
                type: "POST",
    ...
    $("body").delegate(
        ".update_expenses_bgt_walkthrough",
        "click",
        function (e) {
            e.preventDefault();
            ...
            if (name && cat_name && exp_amount && actual_amount) {
                ...
                $(".total_expenses_exp").map(function () {
                    total_exp_amount =
                        total_exp_amount + parseFloat($(this).val());
                });

                $(".total_expenses_act").map(function () {
                    total_act_amount =
                        total_act_amount + parseFloat($(this).val());
                });
    ...
    $("body").delegate(
        ...
        function (e) {
            e.preventDefault();
            non_monthly_expenses_index = $(this).attr(
                "non_monthly_expenses_index"
            );
            ...
            budget_period = $(
                "#non_monthly_expenses_budget_period" +
                    non_monthly_expenses_index
            ).val();
            budget_date = $(
                "#non_monthly_expenses_add_budget_date" +
                    non_monthly_expenses_index
            ).val();
            user_budget_id = $(this).attr("user_budget");
            name = $(
            ...
            exp_amount = $(
                "#non_monthly_expenses_expected_amount" +
                    non_monthly_expenses_index
            ).val();
            actual_amount = $(
    ...
    $("body").delegate(
        ".add_other_non_monthly_expenses",
        "click",
        function (e) {
            last_index = parseInt($(this).attr("last_index")) + 1;
            var user_budget = $(this).attr("user_budget");
    ...
    $("body").delegate(
        ...
        function (e) {
            var last_index = parseInt($(this).attr("index"));
            var budgetDateId =
                "#non_monthly_expenses_add_budget_date" + last_index;
    ...
    $("body").delegate(".add-budget-btn", "click", function (e) {
        ...
        $('#budgetForm [name="account"]').attr(
            "id",
            budget_cat + "_account_id"
        );
    ...
    $("body").delegate(".sub-cat-name", "click", function (e) {
        ...
        if (category === "Income") {
            last_index =
                parseInt($(".add_other_income").attr("last_index")) + 1;
            ...
            last_index =
                parseInt(
                    $(".add_other_non_monthly_expenses").attr("last_index")
                ) + 1;
            ...
            last_index =
                parseInt($(".add_other_expenses ").attr("last_index")) + 1;
    ...
    $("body").delegate("#spentAmountDropdown2", "change", function () {
        ...
        $(".spent-amount2").each(function (index) {
            ...
            $(".remaining-balance2")
                .eq(index)
                .text(remainingBalance.toFixed(2));
        });
    });
