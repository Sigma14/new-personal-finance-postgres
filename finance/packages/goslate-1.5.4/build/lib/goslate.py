...
class Goslate(object):
    ...
    def __init__(
        ...
        if not self._opener:
            ...
            self._opener = build_opener(
                HTTPHandler(debuglevel=debuglevel), HTTPSHandler(
    ...
    def _basic_translate(self, text, target_language, source_language):
        ...
        if not text.strip():
            return tuple("" for i in range(len(self._writing))), unicode(
                target_language
            )
        ...
        GOOGLE_TRASLATE_URL = urljoin(
            random.choice(self._service_urls), "/translate_a/single"
        )
        ...
        raw_data = json.loads(
            _empty_comma.subn("", response_content)[0]
            .replace("\xA0", " ")
            .replace("[,", "[1,")
        )
        ...
        if raw_data[0][-1][0] == 1:  # roman writing
            ...
            data["trans"] = "".join(i[0] for i in raw_data[0][:-1])
        else:
            data["translit"] = ""
            data["trans"] = "".join(i[0] for i in raw_data[0])
    ...
    def _translate_single_text(self, text, target_language, source_lauguage):
        ...
        def make_task(text):
            return lambda: self._basic_translate(
                text, target_language, source_lauguage
            )[0]
    ...
    def translate(self, text, target_language, source_language="auto"):
        ...
        if not _is_sequence(text):
            ...
            return _unwrapper_single_element(
        ...
        return (
            _unwrapper_single_element(i)
            for i in itertools.chain.from_iterable(
                self._execute(make_task(i) for i in join_texts(text))
            )
        )
    ...
    def detect(self, text):
        ...
        if _is_sequence(text):
            return self._execute(
                functools.partial(self._detect_language, i) for i in text
            )
...
def _main(argv):
    ...
    parser = optparse.OptionParser(
        usage=usage, version="%%prog %s @ Copyright %s" % (
